...
自動テストは `tests.html` を開くと実行されます。現在のテスト数は399件です。

## 主な機能

- かんたん設計、プリセット開始、詳細設計の3入口
- 整合済みの開始地点として使えるビルトインプリセット55件（基本ポーズ15件、恋人・親しい相手40件）
...
- 任意の「文字・記号を抑える」「服を保持する」出力補助、四点支持用の白T＋黒ラウンジショーツ補助、背中の開いた服を衣装デザインとして扱うプリセット
...
- 55件すべてのビルトインプリセット、コレクション・撮影範囲・タグの分類と件数
- relationship 40件のメタデータ、相手視点、hard警告、公開向け禁止語
