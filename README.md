...
自動テストは `tests.html` を開くと実行されます。現在のテスト数は206件です。

## 主な機能

- かんたん設計、プリセット開始、詳細設計の3入口
- 整合済みの開始地点として使えるビルトインプリセット30件（アップ10・半身10・全身10）
- 撮影範囲別フィルター、件数表示、相手視点プリセットだけの絞り込み
...
│  ├─ presets.js          30件の整合済みプリセット
│  ├─ preset-utils.js     撮影範囲分類・件数・フィルター
│  └─ rules.js            可視範囲・姿勢別推奨値
├─ tests.html             ブラウザ用テスト画面
├─ tests.js               206件のロジックテスト
└─ README.md
```
...
- 30件すべてのビルトインプリセット、撮影範囲分類、件数、フィルター、重複防止
- 相手視点の正規化、アドバイザー、出力重複防止、旧保存データ移行
- 完成条件
...
- 複数人構図、小道具、具体的な支持物、魚眼、ControlNet連携には対応しません。
