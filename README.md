...
自動テストは `tests.html` を開くと実行されます。現在のテスト数は349件です。

## 主な機能

- かんたん設計、プリセット開始、詳細設計の3入口
- 整合済みの開始地点として使えるビルトインプリセット50件（基本ポーズ15件、恋人・親しい相手35件）
...
- ソファ、ベッド、仰向け・横向き・うつ伏せ、上体支持、背面・振り返り、顔隠し、顔近くの愛情ジェスチャー
...
- 50件すべてのビルトインプリセット、コレクション・撮影範囲・タグの分類と件数
- relationship 35件のメタデータ、相手視点、hard警告、公開向け禁止語
- STEP2雰囲気ボタンの単一選択、状態更新、要約、`aria-pressed`
- 背面強調、表情、顔隠し腕動作の正規化、助言、生成、旧保存データ移行
